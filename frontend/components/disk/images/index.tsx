...
export default function ImagePlayground(props: ImagePlaygroundProps) {
  ...
  const { isOpen, handleEscape, currentIndex, isEdit, handleEdit, onEdit, text, handleInfo } = infoProps
  ...
  return (
    ...
    >
      <div className={clsx(`${isOpen ? 'h-full' : 'hidden'}`, 'lg:flex lg:flex-col justify-around')}>
        <div className='h-full lg:h-[70%]'>
          <div className='flex h-full flex-col lg:flex-row'>
            <div className='bg-white w-full h-full flex items-center overflow-clip'>
              ...
              <span className='absolute top-2 right-2 opacity-60 lg:hidden' onClick={() => handleEscape}>
        ...
        <div className='hidden lg:h-1/5 lg:block lg:mt-[5%] w-full'>
          <div className='h-4/5 imageCarouselContainer'>
            ...
            transition-all opacity-100 duration-300 ease-out ${currentIndex > data.length - 1 ? 'none' : ''}`}
                onClick={() => {
