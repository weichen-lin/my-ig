...
const Label = ({ label, color }: { label: string; color: number }) => {
  ...
  return (
    <li
      className={`py-1 px-2 relative rounded-lg ${color_table[color % 7]} text-sm xs:text-base`}
      onMouseEnter={() => {
        setIshover(true)
      }}
      onMouseLeave={() => {
  ...
  return (
    <div className={clsx('bg-gray-100 w-full h-full p-3 overflow-y-auto flex-col lg:rounded-r-lg')}>
      ...
      <div className='h-10 mt-2'>
        <Edit className='relative h-7 m-[6px] hover:cursor-pointer active:top-[2px]' onClick={handleEdit} />
      </div>
    </div>
  )
}
